...
class EventsDirectoryCatalog(DirectoryCatalog):
    ...
    def limit_subset_to_source(self):
        # Do not limit anything if no source is specified
        if self.import_source == '':
            return

        # Limit subset to external events
        if self.subset:
            # Subset already generated by search/filter
            self.subset = [i for i in self.subset
                           if IExternalEvent.providedBy(i.getObject())]
        else:
            # No subset yet
            self.subset = sorted(
                self.catalog(object_provides=IExternalEvent.__identifier__),
                key=self.sortkey())

        # Limit subset to events with given source
        source = self.path + '/' + self.import_source
        events = [i for i in self.subset if i.getObject().source == source]
        self.subset = events
