...
class TestEventIndex(IntegrationTestCase):
    ...
    def test_lazy_list(self):
        ...
        self.assertEqual(lazy.get_item.call_count, 0)

        lazy[0]
        self.assertEqual(lazy.get_item.call_count, 1)

        lazy[1]
        self.assertEqual(lazy.get_item.call_count, 2)
        ...
        lazy = LazyList(Mock(), 10)
        [i for i in lazy]
        self.assertEqual(lazy.get_item.call_count, 10)
        ...
        lazy = LazyList(Mock(), 10)
        lazy[5:100]
        self.assertEqual(lazy.get_item.call_count, 5)
    ...
    def test_event_order_index(self):
        ...
        self.assertEqual(len(index.by_states(['preview'])), 2)

        event.submit()

        self.assertEqual(len(index.by_states(['preview'])), 1)
        self.assertEqual(len(index.by_states(['submitted'])), 1)
        self.assertEqual(len(index.by_states(['preview', 'submitted'])), 2)
